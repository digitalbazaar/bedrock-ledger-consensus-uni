      ...
      if(options.genesis) {
        ...
        async.auto({
          validate: callback => eventGuard(
            event, event.input[0].eventGuard, callback),
        ...
        async.until(() => done, loopCallback => async.auto({
          ...
          validate: ['getConfig', (results, callback) => eventGuard(
            event, results.getConfig.event.input[0].eventGuard, callback)],
...
function eventGuard(event, guardConfig, callback) {
  ...
  const guards = guardConfig.filter(g => g.supportedEventType === event.type);
