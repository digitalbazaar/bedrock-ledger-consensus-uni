...
api.config = {};
api.config.change = (ledgerConfiguration, ledgerNode, options, callback) => {
  ...
  _addEvent(event, ledgerNode, options, err => callback(err));
};

api.operations = {};
api.operations.add = (operation, ledgerNode, options, callback) => {
  if(typeof options === 'function') {
    callback = options;
    options = {};
  }
...
function _addEvent(event, ledgerNode, options, callback) {
  if(typeof options === 'function') {
    callback = options;
    options = {};
  }
  ...
  async.auto({
    ...
    addBlock: ['writeEvent', (resultsAlpha, callback) => {
      ...
      async.until(() => done, loopCallback => async.auto({
        getLatestBlock: callback => storage.blocks.getLatest(
          options, callback),
